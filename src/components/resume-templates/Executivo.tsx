import { sectionTitles } from "./i18n";
import { ca, stripUrl, resolveOrder, langTerm } from "./shared";
import type { ResumeTemplateProps, SectionName } from "./types";

/**
 * Template Executivo — denso, azul-escuro, para perfis sênior. Resumo com barra
 * lateral, skills rotuladas por categoria (inline tags). Single-column, ATS-safe.
 */
export function Executivo(props: ResumeTemplateProps) {
  const { perfil, curriculo, formacao = [], projetos = [], cursos = [], idiomas = [] } = props;
  const lang = props.lang ?? "pt-BR";
  const t = sectionTitles(lang);
  const skills = curriculo?.habilidades_tecnicas;

  const render = (name: SectionName) => {
    switch (name) {
      case "summary": {
        const text = curriculo?.resumo_profissional || perfil?.resumo_base;
        if (!text) return null;
        return (
          <section key={name}>
            <h2>{t.summary}</h2>
            <p className="summary-text">{ca(text, lang)}</p>
          </section>
        );
      }
      case "experience": {
        const exps = curriculo?.experiencias ?? [];
        if (exps.length === 0) return null;
        return (
          <section key={name}>
            <h2>{t.experience}</h2>
            {exps.map((exp, i) => (
              <article className="experience-item" key={i}>
                <div className="experience-header">
                  <span className="job-title">{ca(exp.cargo, lang)}</span>
                  <span className="period">{exp.periodo}</span>
                </div>
                <div className="company-row">
                  <span className="company-name">{exp.empresa}</span>
                  {exp.localizacao && (
                    <span className="company-location">{exp.localizacao}</span>
                  )}
                </div>
                {exp.bullets && exp.bullets.length > 0 && (
                  <ul className="responsibilities">
                    {exp.bullets.map((b, j) => (
                      <li key={j}>{ca(b, lang)}</li>
                    ))}
                  </ul>
                )}
              </article>
            ))}
          </section>
        );
      }
      case "skills": {
        const rows: Array<[string, string[]]> = [];
        if (skills?.principais?.length) rows.push(["Core", skills.principais]);
        if (skills?.secundarias?.length) rows.push(["Additional", skills.secundarias]);
        if (rows.length === 0) return null;
        return (
          <section key={name}>
            <h2>{t.skills}</h2>
            <div className="skills-container">
              {rows.map(([label, list], i) => (
                <div className="skill-row" key={i}>
                  <span className="skill-category-label">{label}</span>
                  <span className="skill-items">
                    <span className="skills-inline">
                      {list.map((s, j) => (
                        <span className={`skill-tag${i === 0 ? " primary" : ""}`} key={j}>
                          {s}
                        </span>
                      ))}
                    </span>
                  </span>
                </div>
              ))}
            </div>
          </section>
        );
      }
      case "education": {
        if (formacao.length === 0) return null;
        return (
          <section key={name}>
            <h2>{t.education}</h2>
            {formacao.map((f, i) => (
              <article className="education-item" key={i}>
                <div className="education-main">
                  <div className="education-title">{ca(f.titulo_curso, lang)}</div>
                  <div className="education-institution">{f.instituicao_projeto}</div>
                  {f.canadian_equivalency && (
                    <div className="education-institution">
                      {ca(f.canadian_equivalency, lang)}
                    </div>
                  )}
                </div>
                {f.status && <span className="education-year">{langTerm(f.status, lang)}</span>}
              </article>
            ))}
          </section>
        );
      }
      case "certifications": {
        if (cursos.length === 0) return null;
        return (
          <section key={name}>
            <h2>{t.certifications}</h2>
            <div className="certifications-grid">
              {cursos.map((c, i) => (
                <div className="cert-item" key={i}>
                  <span className="cert-name">{ca(c.titulo_do_curso, lang)}</span>
                  <span className="cert-details">{c.emissor_instituicao}</span>
                </div>
              ))}
            </div>
          </section>
        );
      }
      case "languages": {
        if (idiomas.length === 0) return null;
        return (
          <section key={name}>
            <h2>{t.languages}</h2>
            <div className="languages-list">
              {idiomas.map((i, idx) => (
                <span className="language-item" key={idx}>
                  <span className="language-name">{langTerm(i.idioma, lang)}: </span>
                  <span className="language-level">{langTerm(i.nivel_cefr, lang)}</span>
                </span>
              ))}
            </div>
          </section>
        );
      }
      case "projects": {
        if (projetos.length === 0) return null;
        return (
          <section key={name}>
            <h2>{t.projects}</h2>
            {projetos.map((p, i) => (
              <article className="project-item" key={i}>
                <div className="project-title">{p.instituicao_projeto}</div>
                <div className="project-description">{ca(p.descricao_detalhada, lang)}</div>
                {p.tecnologias && <div className="project-tech">{p.tecnologias}</div>}
              </article>
            ))}
          </section>
        );
      }
    }
  };

  return (
    <div className="container">
      <header>
        <h1>{perfil?.nome_completo || "Nome"}</h1>
        {curriculo?.titulo_profissional && (
          <div className="title-role">{ca(curriculo.titulo_profissional, lang)}</div>
        )}
        <div className="contact-info">
          <div className="contact-row">
            {perfil?.email && <span className="contact-item">{perfil.email}</span>}
            {perfil?.telefone && <span className="contact-item">{perfil.telefone}</span>}
            {perfil?.localizacao && <span className="contact-item">{perfil.localizacao}</span>}
            {perfil?.linkedin && (
              <span className="contact-item">
                <a href={perfil.linkedin}>{stripUrl(perfil.linkedin)}</a>
              </span>
            )}
            {perfil?.github && (
              <span className="contact-item">
                <a href={perfil.github}>{stripUrl(perfil.github)}</a>
              </span>
            )}
          </div>
        </div>
      </header>
      {resolveOrder(props.sectionsOrder).map(render)}
    </div>
  );
}
